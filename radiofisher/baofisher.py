...
CAMB_EXEC = "/home/zerui603/.local/bin/" # Directory containing camb executable
...
def figure_of_merit(p1, p2, F, cov=None, twosigma=False):
    ...
    """
    if cov.all() == None: cov = np.linalg.inv(F)
...
def overlapping_expts(expt_in, zlow=None, zhigh=None, Sarea=None):
    ...
    if 'overlap' in list(expt_in.keys()):
        ...
        if Sarea != None: expt_in['Sarea'] = Sarea # Override Sarea
        return expt_in
    ...
    if zlow is None or zhigh is None:
        ...
        if Sarea != None: expt['Sarea'] = Sarea # Override Sarea
        return expt
...
def cached_camb_output(p, cachefile, cosmo=None, mode='matterpower', 
    ...
    for key in keys:
        if key != "output_root":
            yaju_senpai = "%s:%s"%(key, p[key])
            yaju_senpai = yaju_senpai.encode('utf-8')
            m.update(yaju_senpai)
    in_hash = m.hexdigest()
...
def logpk_derivative(pk, kgrid):
    ...
    d log P(k) / d k. Sets the derivative to zero wherever P(k) != defined.
...
def Csignal(q, y, cosmo, expt):
    ...
    f, D = (c['f'], c['D']) if c['f'] != None else fgrowth_k(c, c['z'], k)
...
def indices_for_param_names(paramnames, params, warn=True):
    ...
    warn : bool, optional
        Whether to show a warning if a parameter != found. Default: true.
